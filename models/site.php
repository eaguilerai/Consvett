...
class Site
{
    ...
    $id, $name, $modem_phone, $camera_phone, $modem_state_date, $camera_state_date)
    {
        $this->set_id($id);
        $this->set_name($name);
        $this->set_modem_phone($modem_phone);
        $this->set_camera_phone($camera_phone);
        $this->set_modem_state_date($modem_state_date);
        $this->set_camera_state_date($camera_state_date);
    }
    ...
    private function set_modem_state_date($new_state_date)
    {
        if (\is_string($new_state_date)) {
            $this->m_modem_state_date = \date_create($new_state_date);
        } else {
            $this->m_modem_state_date = $new_state_date;
        }
    }

    private function set_camera_state_date($new_state_date)
    {
        if (\is_string($new_state_date)) {
            $this->m_camera_state_date = \date_create($new_state_date);
        } else {
